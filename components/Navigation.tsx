...
const Navigation = () => {
  ...
  return (
    <div className="relative">
      ...
      >
        <svg
          ...
          className={`hamburger duration-250 transition-all mx-auto ${
            isActive ? "stroke-secondary" : "stroke-primary"
          }`}
      ...
      >
        <ul className="flex flex-col justify-center h-screen w-screen">
          {sections.map((section, index) => (
            <motion.li
              ...
              whileTap={{ scale: 0.9 }}
              key={index}
              className="navigation__item border-t-4 border-primary dark:border-secondary last:border-b-4"
            >
              <a
                href={section.href}
                className="navigation__link font-semibold relative block text-primary dark:text-secondary hover:text-dark"
              >
